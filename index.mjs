...
await assertView("ownerOf", [1], ["None", null]);
...
await assertFail(
  ctc0.a.transferFrom(addr3, addr1, 1),
  "must be existing nft owner or approved operator"
);
await assertFail(
  ctc0.a.transferFrom(addr0, addr1, 1),
  "owner specified in must be correct"
);
...
await ctc0.a.mintTo(addr2);
await assertEvent("Transfer", ["None", null], ["Some", addr2], 2);
await assertView("ownerOf", [2], addr2);
await assertView("tokenURI", [2], params.metadataUriBase + "0002");
await assertView("getApproved", [2], ["None", null]);
await assertView(
  "isApprovedForAll",
  [
    ["Some", addr3],
...
await assertEvent("ApprovalForAll", ["Some", addr3], ["Some", addr1], false); // no longer approved
await assertView(
  "isApprovedForAll",
  [
    ["Some", addr3],
...
await assertFail(ctc1.a.transferFrom(addr3, addr1, 2)); // addr1 can not transfer addr3 nft anymore
...
await ctc3.a.burn(2);
await assertEvent("Transfer", addr3, ["None", null], 2);
